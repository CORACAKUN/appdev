...
return (
  <div className="p-6 max-w-4xl mx-auto space-y-6">
    ...
    <section>
      <h2 className="text-2xl font-semibold text-white mb-2">Your Posts</h2>
      {posts.length === 0 ? (
        <p className="text-white">No posts found.</p>
      ) : (
        <div className="space-y-4">
          {posts.map(post => (
            <div key={post.id} className="p-4 rounded shadow bg-white/1 backdrop-blur-md border-white/30">
              ...
              <div className="mt-4">
                ...
                {comments.filter(comment => comment.postId === post.id).length === 0 ? (
                  <p className="text-white">No comments for this post.</p>
                ) : (
                  <div className="space-y-3">
                    {comments
                      .filter(comment => comment.postId === post.id)
                      .map(comment => (
                        <div key={comment.id} className="p-3 rounded bg-gray-50/20 backdrop-blur-md">
